     ...
     <div class="flex min-h-screen items-start justify-center px-4" @click.self="open = false">
         <div x-show="open" x-transition x-transition.duration.300
             ...
             <div class="flex items-center justify-between bg-[#fbfbfb] px-5 py-3 dark:bg-[#121c2c]">
                 <div class="text-lg font-bold">Ubah Kegiatan </div>
             ...
             <div class="p-5">
                 <form action="{{ route("khs.update", $item->id) }}" method="POST">
                     ...
                     <div class=" mb-3">
                         ...
                         <select name="kelas_id" id="kelas_id_update" class="form-select">
                             @foreach ($class as $kelas)
                                 <option value="{{ $kelas->id }}"
                                     {{ $kelas->id === $item->kelas_id ? "selected" : "" }}>Kelas
                                     {{ $kelas->grade }} - Level {{ $kelas->level }}</option>
                             @endforeach
                         </select>
                     </div>
                     ...
                     <div class="mb-3">
                         ...
                         <input id="nilai" type="number" placeholder="Masukkan Nilai Kegiatan" class="form-input"
                             name="nilai" value="{{ old("nilai", $item->nilai) }}" />
                         @error("nilai")
                             <p class="text-danger mt-1">Mohon dicek Kembali</p>
                         @enderror
                     </div>
